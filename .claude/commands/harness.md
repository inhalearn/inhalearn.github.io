...
**구현 시 반드시 `/docs/IMPLEMENTATION_GUIDE.md`를 먼저 읽어라.**
이 파일에는 다음이 포함되어 있다:

- Phase 0-6 단계별 구현 순서
- 설정 파일 전체 (tailwind.config.js, tsconfig.json, vite.config.ts 등)
- 코드 구현 예시 (Turtle, Interpreter, Validator, Store 등)
- 테스트 코드 예시
- Phase별 검증 기준 및 체크리스트
...
**구현 단계 설계 시**:
1. `/docs/IMPLEMENTATION_GUIDE.md`를 읽어 전체 Phase 구조 파악
2. 현재 구현할 Phase의 상세 항목 확인
3. 필요한 설정 파일 및 코드 예시 참조
